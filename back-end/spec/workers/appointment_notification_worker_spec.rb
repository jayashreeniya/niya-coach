...
RSpec.describe AppointmentNotificationWorker, type: :worker do
  ...
  it "should create notification before uppcomming appointment" do
    ...
    expect { AppointmentNotificationWorker.new.perform(slot.id) }.to change {  BxBlockAppointmentManagement::BookedSlot.where(service_user_id: account.id).count }.by_at_least(0)
  end
end