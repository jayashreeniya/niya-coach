module BxBlockCalendar
  class CoachWithAppointmentSerializer < BaseSerializer


    attributes :upcoming_appointments do |object, params|
      slots = BxBlockAppointmentManagement::BookedSlot.where("service_provider_id = ? and service_user_id = ?", object.id, params[:current_user].id).sort_by(&:start_time)
    ...
    class << self
      ...
      def upcoming_appointments_for(account, slots, params)
        ...
        slots.each do |slot|
          if Time.parse(slot.end_time).localtime > Time.strptime(Time.now.in_time_zone('Asia/Kolkata').to_s, '%Y-%m-%d %H:%M:%S')
            booked_slots<<slot
          end
        end
