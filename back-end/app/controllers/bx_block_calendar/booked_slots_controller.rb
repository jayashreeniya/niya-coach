...
module BxBlockCalendar
  class BookedSlotsController < ApplicationController
    ...
    def create
      ...
      if book_appoint.save
        ...
        availability =  BxBlockAppointmentManagement::Availability.find_by(
                        ...
                        availability_date: book_appoint.booking_date.strftime("%d/%m/%Y"))
    ...
    def cancel_booking
      ...
      availability = BxBlockAppointmentManagement::Availability.find_by("service_provider_id = ? and availability_date = ?", booked_slot.service_provider_id, booked_slot.booking_date.strftime("%d/%m/%Y"))
        availability.available_slots_count += 1
        availability.timeslots.each_with_index do |timeslot, index|
