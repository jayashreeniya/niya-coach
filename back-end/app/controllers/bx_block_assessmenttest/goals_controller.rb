module BxBlockAssessmenttest
	class GoalsController < ApplicationController
		...
		def create
			focus_areas = BxBlockAssessmenttest::StoringFocusArea.where(account_id: @current_user&.id).last&.focus_areas_id
			if focus_areas.present? && focus_areas.flatten.include?(params[:focus_area_id].to_i)
				...
				return render json: {error: "Goal should not be more than 50 letter"},status: :unprocessable_entity if @goal.goal.length >= 50
				utc_datetime = DateTime.parse(@goal.date.to_s).change(hour: Time.parse(@goal.time_slot).hour, min: Time.parse(@goal.time_slot).min).in_time_zone('UTC').to_datetime
