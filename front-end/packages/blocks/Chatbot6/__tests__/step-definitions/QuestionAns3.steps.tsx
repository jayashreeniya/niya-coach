...
const feature = loadFeature('./__tests__/features/QuestionAns-scenario3.feature');
...
jest.mock("@react-native-async-storage/async-storage", () => ({
    getItem: jest.fn((key) => {
        if (key === "answers1") {
          ...
          if(key === "checkedIds"){
            return Promise.resolve(JSON.stringify({"question_id":12,"answers_id":[102,103]}));
          }
    }),
...
test('User will be redirected to last visited screen #3', async({ given, when, then }) => {
