...
const handleError=(isMarkerVariant:boolean,min:number,max:number)=>{
  if (isMarkerVariant)
      if (min < 0) {
...
const getMarks=(isMarkerVariant:boolean,min:number,max:number,step:number)=>{
  if (isMarkerVariant) {
...
const useRadialSlider = (props: RadialSliderHookProps) => {
  ...
  useEffect(() => {
    handleError(isMarkerVariant,min,max)
  ...
  const marks = useMemo(() => {
    getMarks(isMarkerVariant,min,max,step)
