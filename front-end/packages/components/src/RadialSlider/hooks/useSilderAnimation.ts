...
const getNvalue=(step:number,moveStartValue:number,ratio:number,diff:number)=>{
 let nValue:any;
  if (step) {
...
const getValue=(props:any,min:number,max:number)=>{
  if(props?.value < min)
    {
      return min;
    }
...
const useSilderAnimation = (props: RadialSliderAnimationHookProps) => {
  ...
  const handlePanResponderMove = (
    ...
    const ratio =
      (moveStartRadian - radian) / ((Math.PI - (radianValue )) * 2);

    const diff = max - min;

    let nValue: any;
   
   nValue = getNvalue(step,moveStartValue,ratio,diff)
