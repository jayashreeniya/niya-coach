...
/** Use string literals: RN 0.65 + Metro can leave PermissionsAndroid.PERMISSIONS unset and crash native with permission is null. */
...
class Permissions {
     ...
     *
     * @param perm - Android manifest permission string (e.g. android.permission.CAMERA).
     */
    _requestPermissionAndroid(perm: string) {
        if (!perm) {
    ...
    query(permissionDesc: PermissionDescriptor) {
        ...
        if (Platform.OS === 'android') {
            const perm =
                permissionDesc.name === 'camera' ? ANDROID_CAMERA : ANDROID_RECORD_AUDIO;
